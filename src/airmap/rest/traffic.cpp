...
airmap::rest::Traffic::Monitor::Monitor(const std::shared_ptr<Logger>& logger, const std::string& flight_id, const std::shared_ptr<mqtt::Client>& client)
  ...
  sa_subscription_id_ = client->async_subscribe(fmt::sprintf("uav/traffic/sa/%s", flight_id_), ::mqtt::qos::exactly_once);
  alert_subscription_id_ = client->async_subscribe(fmt::sprintf("uav/traffic/alert/%s", flight_id_), ::mqtt::qos::exactly_once);
}
...
bool airmap::rest::Traffic::Monitor::handle_suback(std::uint16_t packet_id, std::vector<::boost::optional<std::uint8_t>> subs) {
...
bool airmap::rest::Traffic::Monitor::handle_publish(std::uint8_t, ::boost::optional<std::uint16_t>,
                                                    std::string topic, std::string contents) {
...
airmap::rest::Traffic::Traffic(const std::shared_ptr<Logger>& logger, const std::string& host, std::uint16_t port, Communicator& communicator)
